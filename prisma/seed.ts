...
interface SeedQuestion {
  ...
  tags: string;
  manualHeat: number; // 系统题给一个基础热度，让广场一进来就有排序
}
...
const SYSTEM_EMAIL = "system@interview-agent.local";
...
async function main() {
  ...
  console.log("创建系统用户（题库内置题的归属者）...");
  const systemUser = await prisma.user.upsert({
    where: { email: SYSTEM_EMAIL },
    update: {},
    create: {
      email: SYSTEM_EMAIL,
      // 该账号仅作题目归属，不用于登录（随机不可用密码哈希）
  ...
  console.log(`完成：1 个系统用户，${questions.length} 道公开题。`);
}
